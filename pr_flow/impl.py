...
class impl(gen_basic):
  ...
  def create_page(self, operator, page_num):
    ...
    if self.prflow_params['overlay_type'] == 'hipr':
      ...
      tmp_dict['CELL_ANCHOR']     = 'set_property SCOPED_TO_CELLS { '+self.prflow_params['inst_name']+'/page'+str(page_num)+'_inst } [get_files $page_dcp]'
      tmp_dict['set inst_name']   = 'set inst_name "'+self.prflow_params['inst_name']+'/${page_name}_inst"',
      tmp_dict['set context_dcp'] = 'set context_dcp "../../F001_overlay/ydma/'+self.prflow_params['board']+'/'+self.prflow_params['board']+'_dfx_manual/checkpoint/p_${page_num}.dcp"'
