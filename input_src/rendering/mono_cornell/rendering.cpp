...
void dflow(hls::stream<bit128 >& input2, hls::stream<ap_axiu<512, 0, 0, 0> >& output2){

  //std::cout << ("hello\n") << std::endl;
  ap_axiu<512, 0, 0, 0> out_FB;
  bit32 output[NUM_FB];
  // local variables
  ...
  TRIANGLES: for (bit16 i = 0; i < NUM_3D_TRI/4; i++){
    for(bit16 j=0; j<4; j++){
      bit128 in_tmp = input2.read();
      ...
      size_pixels = zculling( i*4+j, fragment, size_fragment, pixels);
      coloringFB ( i*4+j, size_pixels, pixels, frame_buffer);
    }
  }
  SEND:for (int i=0; i<MAX_X; i++){
    for(int j=0; j<MAX_Y; j+=64){
      for (int k=0; k<64; k++){
        #pragma HLS PIPELINE II=1
        out_FB.data( k*8+7,  k*8) = frame_buffer[i][j+k];
      }
      output2.write(out_FB);
    }
  }
}
 

void read_in(hls::stream<ap_axiu<512, 0, 0, 0> >& Input_1,
  ...
  for (bit16 i = 0; i < NUM_3D_TRI/4; i++){
    ...
    for(int j=0; j<4; j++){
      #pragma HLS PIPELINE II=1
      out_tmp = in_tmp.data(128*j+127, 128*j);
...
void rendering(hls::stream<ap_axiu<512, 0, 0, 0> >& input2, hls::stream<ap_axiu<512, 0, 0, 0> >& output2){
