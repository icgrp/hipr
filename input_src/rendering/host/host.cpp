...
int main(int argc, char** argv) {
    ...
    OCL_CHECK(err, cl::Buffer buffer_input1(context, CL_MEM_READ_ONLY,   sizeof(bit64)  * CONFIG_SIZE, NULL, &err));
    OCL_CHECK(err, cl::Buffer buffer_input2(context, CL_MEM_READ_ONLY,   sizeof(bit512) * INPUT_SIZE,  NULL, &err));
    OCL_CHECK(err, cl::Buffer buffer_input3(context, CL_MEM_READ_ONLY,   sizeof(bit32)  * DEBUG_SIZE,  NULL, &err));
    OCL_CHECK(err, cl::Buffer buffer_output1(context, CL_MEM_WRITE_ONLY, sizeof(bit64)  * CONFIG_SIZE, NULL, &err));
    OCL_CHECK(err, cl::Buffer buffer_output2(context, CL_MEM_WRITE_ONLY, sizeof(bit512) * OUTPUT_SIZE, NULL, &err));
    OCL_CHECK(err, cl::Buffer buffer_output3(context, CL_MEM_WRITE_ONLY, sizeof(bit32)  * DEBUG_SIZE,  NULL, &err));
    ...
    bit64  *source_input1      = (bit64  *)q.enqueueMapBuffer(buffer_input1,  CL_TRUE, CL_MAP_WRITE, 0, sizeof(bit64)  * CONFIG_SIZE);
    bit512 *source_input2      = (bit512 *)q.enqueueMapBuffer(buffer_input2,  CL_TRUE, CL_MAP_WRITE, 0, sizeof(bit512) * INPUT_SIZE);
    bit32  *source_input3      = (bit32  *)q.enqueueMapBuffer(buffer_input3,  CL_TRUE, CL_MAP_WRITE, 0, sizeof(bit32)  * DEBUG_SIZE);
    bit64  *source_hw_results1 = (bit64  *)q.enqueueMapBuffer(buffer_output1, CL_TRUE, CL_MAP_WRITE | CL_MAP_READ, 0, sizeof(bit64)  * CONFIG_SIZE);
    bit512 *source_hw_results2 = (bit512 *)q.enqueueMapBuffer(buffer_output2, CL_TRUE, CL_MAP_WRITE | CL_MAP_READ, 0, sizeof(bit512) * OUTPUT_SIZE);
    bit32  *source_hw_results3 = (bit32  *)q.enqueueMapBuffer(buffer_output3, CL_TRUE, CL_MAP_WRITE | CL_MAP_READ, 0, sizeof(bit32)  * DEBUG_SIZE);
...
///////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for(int k=0; k<2; k++){
        OCL_CHECK(err, err = krnl_mem_write3.setArg(1, buffer_output3));
        OCL_CHECK(err, err = krnl_mem_write3.setArg(2, DEBUG_SIZE/2));
        std::vector<cl::Event> debug_write_events(1);
        OCL_CHECK(err, err = q.enqueueTask(krnl_mem_write3, nullptr, &debug_write_events[0]));
        // std::cout << "write3 launched" << std::endl;
        // set_callback(debug_write_events[0], "memwrite3");
        OCL_CHECK(err, err = cl::Event::waitForEvents(debug_write_events));
        // std::cout << "write3 finished" << std::endl;
        kernel_wait_events.resize(0);
        kernel_wait_events.push_back(debug_write_events[0]);

        std::vector<cl::Event> debug_copy_events(1);
        OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_output3}, CL_MIGRATE_MEM_OBJECT_HOST, &kernel_wait_events, &debug_copy_events[0]));
        // std::cout << "copy3 launched" << std::endl;
        OCL_CHECK(err, err = cl::Event::waitForEvents(debug_copy_events));
        // std::cout << "copy3 finished" << std::endl;

        for(int i=0; i<DEBUG_SIZE/2; i++){
        printf("%c", (unsigned int) source_hw_results3[i].range(31, 0));
        }
    }
#endif
