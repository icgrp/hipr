...
Versions     : 1.03
Dependencies : C++
Description  : This is a C++ source to perform Simulated Annealing algorithm
               to floorplan the partial reconfigurable regions.
Revision     :
               Revision 0.01 - File Created
               Revision 0.02 - Update file information
               Revision 0.03 - Support Flip-flops for face detection
comment      :
=============================================================================*/
...
hipr::hipr(cmdParser arg_parser){
	...
	app_name           = arg_parser.app_name;
	initial_value_path = arg_parser.path_prefix + "/" +arg_parser.app_name + "/initial.txt";
	pragma_path        = arg_parser.path_prefix + "/" +arg_parser.app_name + "/pragma.txt";
	dfx_path           = arg_parser.path_prefix + "/" +arg_parser.app_name + "/dfx.txt";
	connect_path       = arg_parser.path_prefix + "/" +arg_parser.app_name + "/connect.txt";
	...
	trial_num_per_0p1  = arg_parser.trial_num;
	position_file      = arg_parser.position_file;
	// number_per_iter    = trial_num_per_0p1/(-log(10)/log(AT_INIT));
	number_per_iter    = trial_num_per_0p1*(log(1/AT_INIT)/log(10));
	this->init_pragma();
	this->init_connect();
	this->init_tile();
	this->init_invalid();
...
bool hipr::find_tile_range(Res res_req, Region &prr, bool checkInvalid){
	...
	while(stillNeed.lut > 0 || stillNeed.ff > 0 || stillNeed.bram18>0 || stillNeed.dsp2>0){
		prr.w++;

		// if x_end excesses the boarder of the chip, return false
		if(prr.w+x >= (int) tiles.size())   { return false; }

		x_end = prr.w+x-1;
		// Do not account the resource of the right-most column, since it will be cut off later
		// nornalize the h by (12h-1)/12 as we will leave last one BRAM, DSP or 5 LUTs for routing
		if(tiles[x_end].tile_l == CLB)    {
			stillNeed.lut -= (num_per_tile.lut * (12*h-1)/12);
			stillNeed.ff  -= (num_per_tile.ff  * (12*h-1)/12);
		}
		if(tiles[x_end].tile_l == BRAM18) { stillNeed.bram18 -= (num_per_tile.bram18 * (12*h-1)/12); }
		if(tiles[x_end].tile_l == DSP2)   { stillNeed.dsp2   -= (num_per_tile.dsp2   * (12*h-1)/12); }
		if(tiles[x_end].tile_r == CLB)    {
			stillNeed.lut -= (num_per_tile.lut * (12*h-1)/12);
			stillNeed.ff  -= (num_per_tile.ff  * (12*h-1)/12);
		}
		if(tiles[x_end].tile_r == BRAM18) { stillNeed.bram18 -= (num_per_tile.bram18 * (12*h-1)/12); }
		if(tiles[x_end].tile_r == DSP2)   { stillNeed.dsp2   -= (num_per_tile.dsp2   * (12*h-1)/12); }
	...
	// if the width is less than minumum colomn number, set it to MIN_WIDTH
	prr.w = prr.w<MIN_WIDTH ? MIN_WIDTH : prr.w;
	...
	if(HW_RATIO*prr.h>prr.w) { prr.w = HW_RATIO*prr.h; }
...
double hipr::cost_function(bool debug){
	...
	for(uint i=0; i<dfxs.size(); i++){
		for(int row=dfxs[i].placement.y; row<dfxs[i].placement.y+dfxs[i].placement.h; row++){
			for(int col=dfxs[i].placement.x; col<dfxs[i].placement.x+dfxs[i].placement.w+SAFE_GAP; col++){
				cost_table[row][col]++;
			}
		}
	}
	...
	overlap = overlap < 1 ? overlap : (overlap/(this->tiles.size()*this->max_row*this->dfxs.size())+1.0);
	overlap *= 1000;
	...
	for(unsigned int i=0; i<connects.size(); i++){
		...
		//cout << connects[i].src << "->" << connects[i].dest << ": x=" << manhattan_x << ", y=" << manhattan_y << ", WLcost=" << WLcost << endl;
	}
	...
	for(unsigned int i=0; i<connects.size(); i++){
		int src_num=dfxs_dict[connects[i].src];
		int dest_num=dfxs_dict[connects[i].dest];
		double xl1=dfxs[src_num].placement.x;
		double xr1=dfxs[src_num].placement.x+dfxs[src_num].placement.w-1;
		double yb1=dfxs[src_num].placement.y;
		double yt1=dfxs[src_num].placement.y+dfxs[src_num].placement.h-1;
		double xl2=dfxs[dest_num].placement.x;
		double xr2=dfxs[dest_num].placement.x+dfxs[dest_num].placement.w-1;
		double yb2=dfxs[dest_num].placement.y;
		double yt2=dfxs[dest_num].placement.y+dfxs[dest_num].placement.h-1;

		double xmargin=5;
		double ymargin=0;

		if(xr2+1==xl1){
			if(yt2 <= yt1 && yt2 >= yb1+ymargin){
				b = 0;
			}else if(yb2 <= yt1-ymargin && yb2 >= yb1){
				b = 0;
			}else{
				b = connects[i].width;
			}
			...
			if(xr2 <= xr1 && xr2 >= xl1+xmargin){
				b = 0;
			}else if(xl2 <= xr1-xmargin && xl2 >= xl1){
				b = 0;
			}else{
				b = connects[i].width;
			}
    ...
    */
	...
	for(uint n=0; n<dfxs.size(); n++){
		...
		resource_wastage.ff     += resource.ff     - dfxs[n].resource.ff;
		resource_wastage.bram18 += resource.bram18 - dfxs[n].resource.bram18;
		resource_wastage.dsp2   += resource.dsp2   - dfxs[n].resource.dsp2;
	}
	double Rcost=0;
	Rcost= (resource_wastage.lut+resource_wastage.ff+resource_wastage.bram18+resource_wastage.dsp2)/(device_resource.lut+device_resource.ff+device_resource.bram18+device_resource.dsp2);
...
void hipr:: report_requirement(void){
	Res sum;
	Region shape;
	for (Dfx op: dfxs){
		sum.lut    += op.resource.lut;
		sum.ff     += op.resource.ff;
		sum.bram18 += op.resource.bram18;
		sum.dsp2   += op.resource.dsp2;
	}
	shape.x = 0;
	shape.y = 0;
	shape.w = tiles.size();
	...
	cout << "LUT  total: "               << sum.lut                 << endl;
	cout << "FF   total: "               << sum.ff                  << endl;
	cout << "BRAM total: "               << sum.bram18              << endl;
	cout << "DSP2 total: "               << sum.dsp2                << endl;
	cout << "LUT  device: "              << total.lut               << endl;
	cout << "FF   device: "              << total.ff                << endl;
	cout << "BRAM device: "              << total.bram18            << endl;
	cout << "DSP2 device: "              << total.dsp2              << endl;
	cout << "LUT  Requirement: "         << sum.lut/total.lut       << endl;
	cout << "FF   Requirement: "         << sum.lut/total.ff        << endl;
	cout << "BRAM Requirement: "         << sum.bram18/total.bram18 << endl;
	cout << "DSP2 Requirement: "         << sum.dsp2/total.dsp2     << endl;
	out_log_file << "LUT  Requirement: " << sum.lut/total.lut       << endl;
	out_log_file << "FF   Requirement: " << sum.lut/total.ff        << endl;
	out_log_file << "BRAM Requirement: " << sum.bram18/total.bram18 << endl;
	out_log_file << "DSP2 Requirement: " << sum.dsp2/total.dsp2     << endl;
...
void hipr:: report_utilization(void){
	...
	out_log_file << "LUT  Utilization: " << sum.lut/total.lut << endl;
	out_log_file << "BRAM Utilization: " << sum.bram18/total.bram18 << endl;
	out_log_file << "DSP2 Utilization: " << sum.dsp2/total.dsp2 << endl;
...
void hipr::write_position(){
	string position_path = this->position_file;
	...
	while (Stack.empty() == false) {
		...
		if (u!=0 && u!=(int)dfxs.size()+1){
			if( dir=="H"){
				...
				new_weight = dfxs[node_num].placement.w+SAFE_GAP;
			}else{
...
bool hipr::milp(bool debug){
	...
	Res d[part.size()];
	for(unsigned int p=0; p<part.size(); p++){
		if(part[p].name == CLB)   { d[p].lut = num_per_tile.lut*2;      d[p].ff = num_per_tile.ff*2; }
		if(part[p].name == BRAM18){ d[p].bram18 = num_per_tile.bram18;  d[p].lut = num_per_tile.lut; d[p].ff = num_per_tile.ff; }
		if(part[p].name == DSP2)  { d[p].dsp2 = num_per_tile.dsp2;      d[p].lut = num_per_tile.lut; d[p].ff = num_per_tile.ff; }
		if(part[p].name == URAM)  { d[p].lut = num_per_tile.lut; d[p].ff = num_per_tile.ff; }
		if(part[p].name == IOB)   { d[p].lut = num_per_tile.lut; d[p].ff = num_per_tile.ff; }
		if(part[p].name == PCIE)  { d[p].lut = num_per_tile.lut; d[p].ff = num_per_tile.ff; }
	}
	...
	try {
		...
		env.set("LogFile", "milp_"+this->out_log_path);
		env.start();
		...
		// add constraints
		this->load_init();
		// Minumum Width should be 4
		for(uint n=0; n<dfxs.size(); n++){ model.addConstr(w[n] >= MIN_WIDTH); }
		...
		for(uint n1=0; n1<dfxs.size(); n1++){
			for(uint n2=n1+1; n2<dfxs.size(); n2++){
				model.addConstr(x[n1] + w[n1] + SAFE_GAP <= x[n2] + g[n1][n2]*tiles.size());
			}
		}
		...
		for(uint n=0; n<dfxs.size(); n++){
			...
			model.addConstr(lhs_ff     >= dfxs[n].resource.ff    );
		...
		for(int r=0; r<max_row; r++){
			for(uint n1=0; n1<dfxs.size(); n1++){
				for(uint n2=n1+1; n2<dfxs.size(); n2++){
					model.addConstr(x[n1] >= x[n2]+w[n2]+SAFE_GAP-(3-g[n1][n2]-a[n1][r]-a[n2][r])*tiles.size());
		...
		for(uint n=0; n<dfxs.size(); n++){
			...
			lhs_ff     -= dfxs[n].resource.ff;
			lhs_bram18 -= dfxs[n].resource.bram18;
			lhs_dsp2   -= dfxs[n].resource.dsp2;
		}

		Rcost = (lhs_lut+lhs_ff+lhs_bram18+lhs_dsp2)/(device_resource.lut+device_resource.ff+device_resource.bram18+device_resource.dsp2);
		...
		// Optimize model
		if(MILP_MAX_TIME != 0){ model.set(GRB_DoubleParam_TimeLimit, MILP_MAX_TIME); }
		...
		cout << "======================================" << endl;
		cout << "Obj (MILP): " << model.get(GRB_DoubleAttr_ObjVal) << endl;
		cout << "Cost(HiPR): " << this->cost_function() << endl;


		this->print_dfxs();
		this->gen_xdc();
		this->write_position();
		this->report_requirement();
...
void hipr::dump_result(bool isSuccess){
	string result_path;
	if (isSuccess){
		result_path = "./"+this->app_name+"_"+this->device+".success";
	}else{
		result_path = "./"+this->app_name+"_"+this->device+".fail";
	}
	const char *cstr = result_path.c_str();
	ofstream myfile;
	myfile.open (cstr);
	myfile.close();
}
...
void hipr::gen_xdc(void){
	...
	for(uint i=0; i<dfxs.size(); i++){
		if(dfxs[i].isPR == "hipr" || dfxs[i].isPR == "HIPR"){
			Res_range out;
			...
			if (out.valid){
				...
				// myfile << out.start << "Y" << CLB_OFFSET+dfxs[i].placement.y*60+5 << ":SLICE_X" << out.end << "Y" << CLB_OFFSET+(dfxs[i].placement.y+dfxs[i].placement.h)*60-1 << "}" << endl;
				myfile << out.start << "Y" << CLB_OFFSET+dfxs[i].placement.y*60 << ":SLICE_X" << out.end << "Y" << CLB_OFFSET+(dfxs[i].placement.y+dfxs[i].placement.h)*60-1 << "}" << endl;
			}
			...
			if (out.valid){
				...
				int left_botton_Y = DSP2_OFFSET+dfxs[i].placement.y*24 < 0 ? 0 : DSP2_OFFSET+dfxs[i].placement.y*24;
			...
			if (out.valid) {
				...
				left_botton_Y = BRAM18_OFFSET+dfxs[i].placement.y*24 < 0 ? 0 : BRAM18_OFFSET+dfxs[i].placement.y*24;
				myfile << out.start << "Y" << left_botton_Y << ":RAMB18_X" << out.end << "Y" << BRAM18_OFFSET+(dfxs[i].placement.y+dfxs[i].placement.h)*24-1 << "}" << endl;
				...
				left_botton_Y = BRAM36_OFFSET+dfxs[i].placement.y*12 < 0 ? 0 : BRAM36_OFFSET+dfxs[i].placement.y*12;
				myfile << out.start << "Y" << left_botton_Y << ":RAMB36_X" << out.end << "Y" << BRAM36_OFFSET+(dfxs[i].placement.y+dfxs[i].placement.h)*12-1 << "}" << endl;
			}
			...
			if (out.valid){
				...
				int left_botton_Y = URAM288_OFFSET+dfxs[i].placement.y*16 < 0 ? 0 : URAM288_OFFSET+dfxs[i].placement.y*16;
			...
			myfile << "] [get_cells -quiet [list " << this->xdc_inst << "/increment_1/mono_inst/" << dfxs[i].name << "_inst]] -quiet" << endl;
		}
	}
	myfile.close();
}
...
bool hipr::SimulatedAnnealing(bool debug){
	...
	Dfx op_backup;                                                          // backup the operator for one Annealing

	// Sequence pair for floorplan representation
	vector<int> pos_seq;
	vector<int> neg_seq;

	int    func_idx1       = 0;                                            // initialize function index 1
	int    func_idx2       = 1;                                            // initialize function index 2
	int    swap_case       = 0;
	double T               = t_init;
	int    L               = 0;
	int    L_per_T         = 0;
	...
	// Calculate the maximum global trial times to predict the completion percentage
	int iter_max = (int)log10(t_init/t_min)*this->trial_num_per_0p1+this->trial_num_per_0p1*4;


	#ifdef INIT
		// If INIT is define, load the intial values from the init file
		// load the initial floorplan
		this->load_init();
		cout << "Initial Cost=" << this->cost_function() << endl;
	#endif
	...
	// copy the same log  into log file
	out_log_file << "Time: 0";
	out_log_file << " T: " << T;
	out_log_file << " Already_tried " << 0 << "%";
	out_log_file << " cost_min = " << cost_min;
	out_log_file << " cout_last = " << cost_last;
	...
	// while((L--) && (stimer.return_dur() < 600)){
		...
		if( (df < 0)) { // if the cost is decreased, accept the swap
			...
			if(opt_solver == "SQSA"){ this->seq_swap(swap_case, func_idx1, func_idx2, pos_seq, neg_seq); }
			if(opt_solver == "XYSA"){ dfxs[func_idx1] = op_backup; }
		}
		...
		if(T>this->t_min){ // If not reach the frozen point, check whether reach the trial number
			if(L_per_T == this->number_per_iter){ // if reach the trial number, reset L_per_T, and decrease the temperature
				at = AT_INIT;
				//if(T>1)        { at = 0.9999;   }
			...
			if(freeze_cnt++ == 4*trial_num_per_0p1){ // try 4*trial_num times at the frozen temperature.
	...
	out_log_file << "L_max " << this->trial_num_per_0p1 << endl;
	...
	cout << "================number_per_iter = " << this->number_per_iter << endl;
 	cout << tiles.size() << endl;
 	if(cost_min < 1){ this->out_log_file << this->opt_solver << " succeeded!" << endl; }
 	else            { this->out_log_file << this->opt_solver << " failed!" << endl;    }
